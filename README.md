...
- Run `pod install` (You first need to install [Cocoapods](https://cocoapods.org/) on your machine to make use of the pod command).
- Open the Sketchnoting .xcworkspace file (NOT project) that has been generated in Xcode: upon first opening the workspace, Xcode may take a while to index the project's files before you can build it.
...
- Run the app!
    - In case you want to run the app on a physical device, open the root 'Sketchnoting' file in the Project Navigator in Xcode and setup an Apple Provisioning Profile under General > Signing.
