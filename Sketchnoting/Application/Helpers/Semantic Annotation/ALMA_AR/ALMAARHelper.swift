...
class ALMAARHelper {
    ...
    func fetch(db_categories: [String], spot: String, note: Note) {
        for category in db_categories {
            let db_concept_sanitized = category.replacingOccurrences(of: " ", with: "_")
            let parameters: Parameters = [:]
            let headers: HTTPHeaders = [
                "Accept": "application/json"
            ]
            AF.request("https://alma.uni.lu/api/concepts/dbr:\(db_concept_sanitized)/resources/", parameters: parameters, headers: headers).responseJSON { response in
                    ...
                    if let matches = json.array {
                        for match in matches {
                            if let id = match["id"].int, let _ = match["type"].string, let _ = match["url"].string {
                                self.fetchResource(id: id, category: category, spot: spot, note: note)
                            }
                        }
                    }
                }
    ...
    private func fetchResource(id: Int, category: String, spot: String, note: Note) {
        let parameters: Parameters = [:]
        let headers: HTTPHeaders = [
            "Accept": "application/json"
        ]
        AF.request("https://alma.uni.lu/api/resources/\(id)/", parameters: parameters, headers: headers).responseJSON { response in
            self.almaarQueue.async {
                ...
                if let id = json["id"].int, let title = json["title"].string, let previewURL = json["previewURL"].string, let url = json["url"].string {
                    if let document = ARDocument(title: title, description: nil, URL: url, type: .ALMAAR, spot: spot, categories: [category], wikiPageID: nil)  { // Wiki page ID same as id?
