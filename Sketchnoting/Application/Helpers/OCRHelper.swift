...
class OCRHelper {
    ...
    static func check(text: String) {
        let firstRun = correct(original: text)
        print("First run: " + firstRun)
        let secondRun = correct(original: firstRun)
        print("Second run: " + secondRun)
    }
    
    private static func correct(original: String) -> String {
